<template>
    <section>
        <h2 class="text-center mt-6">LAWYER FORM</h2>
        <div class="mt-8"></div>
        <v-form @submit.prevent="submit" ref="form">
            <div class="center-form add-form">
                ...
                <v-text-field hint="If you work for a law firm, please provide the name of the firm."  v-model="obj.company" variant="solo" label="Law Firm Name">
                </v-text-field>

                <v-text-field hint="E.g. property transactions, contracts, zoning, litigation, etc."  v-model="obj.expertise" variant="solo" :rules="errorMessage.expertise" label="Describe your expertise and specialization in real estate law*">
                </v-text-field>
    ...
    methods: {
        ...
        async submit() {
            ...
            if (validate.valid) {
                ...
                this.$store.dispatch('addLegalData', this.obj);
                this.snackbar = true;
                setTimeout(() => {
                    this.handleReset();
                    this.snackbar = false;
