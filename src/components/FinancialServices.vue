<template>
    <section>
        <h2 class="text-center mt-6">BANKER/LOAN/FINANCIAL SERVICE FORM</h2>
        <div class="mt-8"></div>
        <v-form @submit.prevent="submit" ref="form">
            <div class="center-form add-form">
                ...
                <v-text-field hint="If you work for a Bank or an Organization, please provide the name"  v-model="obj.company" variant="solo" label="Bank / Organization Name">
                </v-text-field>

                <v-text-field hint="E.g. property loans etc."  v-model="obj.expertise" variant="solo" :rules="errorMessage.expertise" label="Describe your expertise and specialization in real estate Finance*">
                </v-text-field>
...
export default {
    data() {
        return {
            ...
            errorMessage: {
                ...
                expertise: [v => !!v || 'Please Enter your Expertise Details'],
                email: [
                    value => {
    ...
    methods: {
        async sendMail(obj) {
            let name = obj.name;
            let email = obj.email;
            let senderType = 'Financial Services';
        ...
        async submit() {
            ...
            if (validate.valid) {
                ...
                this.$store.dispatch('addFinanceData', this.obj);
                this.snackbar = true;
                setTimeout(() => {
                    this.handleReset();
                    this.snackbar = false;
