...
function doGet() {
  ...
  Logger.log("2")
  const data = firestore.query("developer").OrderBy('timestamp').Execute();
  Logger.log(data[0]);
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    Logger.log(check);
    if (check) {
      let projectName = data[i]?.fields?.projectName?.stringValue;
      let builderName = data[i]?.fields?.builderName?.stringValue;
      let location = data[i]?.fields?.location?.stringValue;
      let rera = data[i]?.fields?.rera?.stringValue;
      let projectStatus = data[i]?.fields?.projectStatus?.stringValue;
      let projectOverview = data[i]?.fields?.projectStatus?.stringValue;
      let ammenities = data[i]?.fields?.ammenities?.stringValue;
      let features = data[i]?.fields?.features?.stringValue;
      let unitSizes = data[i]?.fields?.unitSizes?.stringValue;
      let priceRange = data[i]?.fields?.priceRange?.stringValue;
      let possesionDate = data[i]?.fields?.possesionDate?.stringValue;
      let legalApprovals = data[i]?.fields?.legalApprovals?.stringValue;
      let salesContact = data[i]?.fields?.salesContact?.stringValue;
      let email = data[i]?.fields?.email?.stringValue;
      let website = data[i]?.fields?.website?.stringValue;
      let additional = data[i]?.fields?.additional?.stringValue;

      let projectTypeArray = data[i]?.fields?.projectType?.arrayValue.values;
      let projectType = ''
      for (let i = 0; i < projectTypeArray.length; i++) {
        projectType += projectTypeArray[i].stringValue + ',';
      }
      let lastIndex = projectType.lastIndexOf(',');
      projectType = projectType.slice(0, lastIndex) + ' ' + projectType.slice(lastIndex + 1);

      let timestamp = data[i]?.fields?.timestamp?.timestampValue;
      var sheet = ss.getSheets()[0];
      sheet.appendRow([id, projectName, builderName, location, rera, projectType, projectStatus, projectOverview, ammenities, features, unitSizes, priceRange, possesionDate, legalApprovals, salesContact, email, website, additional, timestamp]);
    }
