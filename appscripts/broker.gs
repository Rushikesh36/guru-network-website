...
function doGet() {
  ...
  Logger.log("2")
  const data = firestore.query("broker").OrderBy('timestamp').Execute();
  Logger.log(data[0]);
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (check) {
      ...
      let contact = data[i]?.fields?.contact?.stringValue;
      let company = data[i]?.fields?.company?.stringValue;
      let expertise = data[i]?.fields?.expertise?.stringValue;
      let experience = data[i]?.fields?.experience?.stringValue;

      let timestamp = data[i]?.fields?.timestamp?.timestampValue;
      var sheet = ss.getSheets()[0];
      sheet.appendRow([id, name, location, email, contact, company, expertise, experience, timestamp]);
    }
