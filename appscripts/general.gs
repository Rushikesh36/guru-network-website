...
function doGet() {
  ...
  Logger.log("2")
  const data = firestore.query("general").OrderBy('timestamp').Execute();
  Logger.log(data[0]);
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (check) {
      ...
      let message = data[i]?.fields?.message?.stringValue;
      let timestamp = data[i]?.fields?.timestamp?.timestampValue;
      var sheet = ss.getSheets()[0];

      sheet.appendRow([id, name, email, phoneNumber, message, timestamp]);
    }
