...
function doGet() {
  ...
  Logger.log("2")
  const data = firestore.query("buyer").OrderBy('timestamp').Execute();
  Logger.log(data[0]);
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (check) {
      ...
      let locationPreferences = data[i]?.fields?.locationPreferences?.stringValue;
      let additional = data[i]?.fields?.additional?.stringValue;
      let otherRequirement = data[i]?.fields?.otherRequirement?.stringValue;
      let otherBudget = data[i]?.fields?.otherBudget?.stringValue;
      let otherFacility = data[i]?.fields?.otherFacility?.stringValue;
      let phoneNumber = data[i]?.fields?.phoneNumber?.stringValue;
      let email = data[i]?.fields?.email?.stringValue;
      let flatTypeArray = data[i]?.fields?.flatType?.arrayValue.values;
      let flatType = ''
      for (let i = 0; i < flatTypeArray.length; i++) {
        flatType += flatTypeArray[i].stringValue + ',';
      }
      let lastIndex = flatType.lastIndexOf(',');
      flatType = flatType.slice(0, lastIndex) + ' ' + flatType.slice(lastIndex + 1);
      let budgetArray = data[i]?.fields?.budget?.arrayValue.values;
      let budget = '';
      for (let i = 0; i < budgetArray.length; i++) {
        budget += budgetArray[i].stringValue + ',';
      }
      lastIndex = budget.lastIndexOf(',');
      budget = budget.slice(0, lastIndex) + ' ' + budget.slice(lastIndex + 1);
      let facilityArray = data[i]?.fields?.facility?.arrayValue.values;
      let facility = ''
      for (let i = 0; i < facilityArray.length; i++) {
        facility += facilityArray[i].stringValue + ',';
      }
      lastIndex = facility.lastIndexOf(',');
      facility = facility.slice(0, lastIndex) + ' ' + facility.slice(lastIndex + 1);
      let moveinArray = data[i]?.fields?.movein?.arrayValue.values;
      let movein = '';
      for (let i = 0; i < moveinArray.length; i++) {
        movein += moveinArray[i].stringValue + ',';
      }
      lastIndex = movein.lastIndexOf(',');
      movein = movein.slice(0, lastIndex) + ' ' + movein.slice(lastIndex + 1);
      let timestamp = data[i]?.fields?.timestamp?.timestampValue;
      var sheet = ss.getSheets()[0];
      sheet.appendRow([id, name, locationPreferences, phoneNumber, email, flatType, otherRequirement, budget, otherBudget, facility, otherFacility,  movein, additional,  timestamp]);
    }
